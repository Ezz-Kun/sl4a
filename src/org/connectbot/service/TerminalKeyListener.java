...
public class TerminalKeyListener implements OnKeyListener,
	...
	public final static int KEYBOARD_META_CTRL_ON = 0x1000; // Ctrl key mask for
															// API 11+
	private final TerminalManager manager;
	private final TerminalBridge bridge;
	private final VDUBuffer buffer;

	protected KeyCharacterMap keymap = KeyCharacterMap
	...
	public TerminalKeyListener(TerminalManager manager, TerminalBridge bridge,
			VDUBuffer buffer, String encoding) {
		...
		hardKeyboard = (manager.getResources().getConfiguration().keyboard == Configuration.KEYBOARD_QWERTY);
	...
	public boolean onKey(View v, int keyCode, KeyEvent event) {
		try {
			...
			if (event.getAction() == KeyEvent.ACTION_UP) {
				...
				// skip keys if we aren't connected yet or have been
				// disconnected
			...
			if (printing) {
				...
				// Doesn't work properly with asus keyboards... may never have
				// worked. RM 09-Apr-2012
				/*
				 * if ((hardKeyboard && !hardKeyboardHidden) && (curMetaState &
				 * KeyEvent.META_SHIFT_ON) != 0 && sendFunctionKey(keyCode)) {
				 * return true; }
				 */
				...
				if (key < 0x80) {
					...
					// TODO write encoding routine that doesn't allocate each
					// time
			...
			case KeyEvent.KEYCODE_CAMERA:
				...
				String camera = manager.getStringParameter(
	...
	private void updateKeymode() {
		keymode = manager.getStringParameter(PreferenceConstants.KEYMODE,
				PreferenceConstants.KEYMODE_RIGHT);
	}

...
public class TerminalView extends View implements FontSizeChangedListener {
	...
	public TerminalView(Context context, TerminalBridge bridge) {
		...
		cursorPaint.setXfermode(new PixelXorXfermode(bridge
				.getBackgroundColor()));
	...
	public void onDraw(Canvas canvas) {
		if (bridge.getBitmap() != null) {
			...
			if (buffer.isCursorVisible()) {
				...
				int currentAttribute = buffer.getAttributes(cursorColumn,
						cursorRow);
				boolean onWideCharacter = (currentAttribute & VDUBuffer.FULLWIDTH) != 0;

				int x = cursorColumn * bridge.charWidth;
				int y = (buffer.getCursorRow() + buffer.screenBase - buffer.windowBase)
						* bridge.charHeight;
				...
				canvas.clipRect(0, 0, bridge.charWidth
						* (onWideCharacter ? 2 : 1), bridge.charHeight);
			...
			if (bridge.isSelectingForCopy()) {
				...
				canvas.clipRect(area.getLeft() * bridge.charWidth,
	...
	/**
	 * Ask the {@link TerminalBridge} we're connected to to resize to a specific
	 * size.
	 * 
	 * @param width
	 * @param height
	...
	/**
	 * Sets the ability for the TerminalView to display Toast notifications to
	 * the user.
	 * 
	 * @param value
